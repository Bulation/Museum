...
export class Slider {
  ...
  changeSrc(n) {
    ...
    Video.video.src = `assets/videos/${n}.mp4`;
    Video.video.poster = `assets/img/posters/${n}.jpg`;
    Video.endingVideo();
  }
  clonePrepend(ind) {
  ...
  slideElements(n) {
    ...
    if (this.container == videoSliderContainer)
        this.changeSrc(this.currentSlide)
  }
  ...
  swipeDetect(el) {
    ...
    el.addEventListener("touchend", (e) => {
      ...
      finishTime = new Date();
      if (
        distanceX > allowedDistanceX &&
        distanceY < allowedDistanceY &&
        finishTime - startTime > allowedTime &&
        this.isEnabled
      ) {
